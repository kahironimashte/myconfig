...
;; -----------
 ...
 (global-set-key (kbd "<S-f12>") 'kahiro-agenda-next-buffer-tasks) 

(defun kahiro-agenda-next-xa-tasks (&optional arg) ;; Shift+<f12>
  (interactive "P")
...
#+BEGIN_SRC emacs-lisp  :results output silent
  ...
  (setq org-capture-templates
	...
	:empty-lines-after 1)
      ("t" "Termin in Inbox Arbeit" entry (file+headline (lambda () (concat org-directory "/atasks.org")) "aInbox")
	"** %^{Status|PLAN|TODO|SPLIT|PROJ} [#%^{Priorität eintragen|C|B|A|D}] %?%^{Aufgabe} %^g \n DEADLINE: %^t  \n  - Anmerkungen :: %^{Anmerkungen}" 
	:empty-lines-after 1)
	...
	"** MEET [#%^{Priorität?|C|B|A|D}] %^{was für eine Besprechung?} :meet:%^g \n %^t \n  - [[file:arbeit/meetingnotizen.org::*Mitarbeiter-Info][Meeting-Notizen]]\n  - Anmerkungen :: %^{Anmerkungen}\n%?")
      ("s" "Störungen wie Anruf/Kollege")
      ("sa" "Anruf" entry (file+headline (lambda () (concat org-directory "/atasks.org")) "Unterbrechungen")
	"** %^{Anrufer} %U :call:interrupt: \n - Anmerkungen :: %^{Anmerkungen}%?"
	:prepend t :clock-in t :clock-resume t :empty-lines-after 1)
      ("ss" "Unterbrechung" entry (file+headline (lambda () (concat org-directory "/atasks.org")) "Unterbrechungen")
	"** %?%^{Unterbrecher} %U :interrupt: \n - Anmerkungen :: %^{Anmerkungen}"
...
** Tags
#+begin_src emacs-lisp :results output silent
(setq org-tags-exclude-from-inheritance '("Autor" "Fokus" "TB" "leer" "kur" "gut"))
